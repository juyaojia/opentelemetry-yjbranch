...
func (c *Config) Flags(fs *flag.FlagSet) {
        ...
        fs.IntVar(&c.NumTraces, "traces", 2, "Number of traces to generate in each worker (ignored if duration is provided")
        fs.IntVar(&c.TotalTraceTypes, "types", 2, "Number of trace types to generate in each worker")
